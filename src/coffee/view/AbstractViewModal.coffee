...
class AbstractViewModal extends AbstractView
	...
	show : (cb) =>
		...
		@CD_CE().appView.wrapper.addChild @
	...
	close : =>
		...
		@CD_CE().router.navigateTo @CD_CE().appView.wrapper.backgroundView.route
	...
	animateInDone : (cb) =>
		...
		@CD_CE().appView.modalPlayBtn.show()

		null

	animateOut : (cb) =>

		@CD_CE().appView.modalPlayBtn.hide()
